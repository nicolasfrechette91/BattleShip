...
| `{"protocol":1,"op":"status"}` | `{"protocol":1,"op":"status","ok":true,"state":S,"state_name":"...","can_step":bool,"step_count":N,"no_render":bool,"raphnet_disabled":bool}` (`no_render` is an M6 additive field: the process's constant host mode, `SSB64_RL_NO_RENDER` effective or not; `raphnet_disabled` is an M6 follow-up additive field: whether the process-local native Raphnet adapter bypass `SSB64_RAPHNET_DISABLE` was kept for the process; both are configuration, not observations, see `docs/rl_throughput_m6.md`) |
